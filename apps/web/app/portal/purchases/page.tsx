  ...
  return (
    <main className="customer-page">
      ...
      <section className="customer-intro">
        <span className="market-eyebrow">YOUR PROTECTED COMMERCE</span>
        <h1>My purchases</h1>
        <p>Every payment stays protected until you confirm the work is complete.</p>
      </section>
      <div className="purchase-tabs">
        <span className="selected">All transactions</span>
        <span>Active</span>
        <span>Completed</span>
        <span>Disputed</span>
      </div>
      <section className="purchase-list">
        {deals.map((deal) => (
          <Link className="purchase-row" href={`/portal/deals/${deal.id}`} key={deal.id}>
            <div className="purchase-symbol">↗</div>
            <div className="purchase-main">
              <strong>{deal.title}</strong>
              <span>
                {deal.reference} · {new Date(deal.createdAt).toLocaleDateString('en-GH')}
              </span>
            </div>
        ...
        {!deals.length && (
          <div className="customer-empty">
            ...
            <span>When you create a protected Deal, its timeline will appear here.</span>
            <Link href="/marketplace">Browse trusted sellers →</Link>
          </div>
        )}
      </section>
    </main>
