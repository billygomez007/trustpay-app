    ...
    return (
      <main className="marketplace">
        ...
        <div className="market-empty">
          {error ? (
            <>
              <strong>Seller profile unavailable</strong>
              <span>This Trust Profile may be private or no longer available.</span>
  ...
  return (
    <main className="storefront">
      ...
      <section className="storefront-hero">
        <div className="storefront-avatar">{seller.displayName.slice(0, 1).toUpperCase()}</div>
        <div>
          <span className="verified-badge">✓ {seller.verificationLevel.replaceAll('_', ' ')}</span>
          <h1>{seller.displayName}</h1>
          <p>
            {seller.country || 'Ghana'} · On TrustPay since{' '}
      ...
      <section className="storefront-body">
        <div className="storefront-main">
          <span className="market-eyebrow">SELLER STOREFRONT</span>
          <h2>Trusted commerce, clearly recorded.</h2>
          <p className="storefront-copy">
            This seller has opted into a public Trust Profile. Product and service listings will
            appear here when the marketplace catalog is available.
          </p>
          <div className="storefront-empty">
            <span>◇</span>
            <strong>No listings yet</strong>
            <p>TrustPay never invents products, prices, ratings, or availability.</p>
          </div>
        </div>
        <aside className="trust-card">
          <span className="market-eyebrow">WHY BUYERS TRUST THEM</span>
