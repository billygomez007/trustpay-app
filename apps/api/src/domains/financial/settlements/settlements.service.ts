...
export class SettlementsService {
  ...
  public async createForCompletedDeal(actorId: string, dealId: string, idempotencyKey: string) {
    ...
    const settlement = await prisma.$transaction(async (transaction) => transaction.settlement.create({
      data: {
    ...
    await this.audit.record({ actorId, businessId: deal.businessId ?? undefined, action: 'financial.settlement.payable_created', resource: `settlement:${settlement.id}` });
    return settlement;
  }

  public async complete(actorId: string, settlementId: string) {
    await this.requireFinanceRole(actorId);
    const settlement = await prisma.settlement.findUnique({ where: { id: settlementId }, include: { deal: true } });
    if (!settlement) throw new NotFoundException('Settlement not found.');
    if (settlement.status === 'completed') return settlement;
    if (settlement.status !== 'created' && settlement.status !== 'processing') throw new BadRequestException('Settlement is not ready for completion.');
    const result = await prisma.$transaction(async (transaction) => {
      const updated = await transaction.settlement.update({ where: { id: settlementId }, data: { status: 'completed' } });
      await this.ledger.postJournal({
        ...
        lines: [
          { accountCode: `seller-payable-${settlement.currency}`, accountName: 'Seller Payable', accountType: 'liability', direction: 'debit', amount: settlement.amount.toFixed(2), currency: settlement.currency },
          { accountCode: `settlement-clearing-${settlement.currency}`, accountName: 'Settlement Clearing', accountType: 'asset', direction: 'credit', amount: settlement.amount.toFixed(2), currency: settlement.currency }
        ]
      }, transaction);
      return updated;
    });
    await this.audit.record({ actorId, businessId: settlement.deal.businessId ?? undefined, action: 'financial.settlement.completed', resource: `settlement:${settlementId}` });
    return result;
  }
