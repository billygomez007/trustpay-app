...
export class RefundsService {
  ...
  public async request(actorId: string, input: CreateRefundInput) {
    const existingRequest = await prisma.refund.findUnique({
    ...
    const previousMinor = previous.reduce(
      (sum, refund) => sum + toMinor(refund.amount.toFixed(2)),
      0n
    );
  ...
  public async approve(actorId: string, refundId: string) {
    ...
    const refund = await prisma.refund.findUnique({
      where: { id: refundId },
      include: { paymentIntent: { include: { deal: true } } }
    });
    if (!refund) throw new NotFoundException('Refund not found.');
    if (refund.status !== 'requested')
      throw new BadRequestException('Refund is not awaiting approval.');
    const updated = await prisma.$transaction(async (transaction) => {
      const result = await transaction.refund.update({
        where: { id: refundId },
      ...
      await transaction.financialEvent.create({
        data: {
          ...
          source: 'trustpay.refunds',
          metadata: { refundId } as Prisma.InputJsonValue
        }
      });
      return result;
    });
    await this.audit.record({
      actorId,
      businessId: refund.paymentIntent.deal.businessId ?? undefined,
  ...
  public async complete(actorId: string, refundId: string) {
    ...
    if (!['approved', 'processing'].includes(refund.status))
      throw new BadRequestException('Refund is not ready for completion.');
    const completed = await prisma.$transaction(async (transaction) => {
      ...
      await this.ledger.postJournal(
        {
          reference: `JE-${refund.reference}`,
          source: 'refund.completed',
          description: `Refund completed for ${refund.paymentIntent.deal.reference}`,
          dealId: refund.paymentIntent.dealId,
          paymentIntentId: refund.paymentIntentId,
          lines: [
            {
              accountCode: `protected-transaction-${refund.currency}`,
            ...
            {
              accountCode: `customer-payment-${refund.currency}`,
